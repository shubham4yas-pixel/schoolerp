...
const StudentRegistrationForm = ({ schoolId, addStudent, students, role }: { schoolId: string; addStudent: any; students: Student[]; role: string | null }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name || !form.parentName || !form.parentContact) { toast.error('Please fill required fields (Student Name, Parent Name, Parent Contact)'); return; }
      ...
      .insert([
        {
          ...
          parent_contact: newStudent.parentContact,
          mother_name: (newStudent as any).motherName || '',
          address: (newStudent as any).address || '',
          date_of_birth: (newStudent as any).dateOfBirth || null,
          blood_group: (newStudent as any).bloodGroup || '',
          transport_enabled: busOpted,
          uses_bus: busOpted,
    ...
    // Refresh students list after successful registration
    const { fetchStudents: refresh } = useStore.getState();
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Input label="Mother's Name" value={form.motherName} onChange={v => setForm({ ...form, motherName: v })} placeholder="Mother's full name" />
